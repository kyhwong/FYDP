    ...
    def setupPosix(self):
        ...
        while self._goOn:
            try:
                ...
                if data != "":
                    if _os_decryption:
                        self.packets.put_nowait(EmotivPacket(data, self.sensors))
                        gevent.sleep(0)
                    else:
                        # Queue it!
                        self.packetsReceived += 1
